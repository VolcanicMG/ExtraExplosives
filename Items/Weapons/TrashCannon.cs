    ...
    public class TrashCannon : ExplosiveWeapon
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = 0; i < numberProjectiles; i++)
            {
                ...
                Projectile.NewProjectile(source, position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<TrashCannonProjectile>(), damage, knockback, player.whoAmI);
            }

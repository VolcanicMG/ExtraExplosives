...
{
	public class BulletBoomProjectileHyperius : ModProjectile
	{
				...
				for (int y = -radius; y <= radius; y++)
				{
					...
					if (Math.Sqrt(x * x + y * y) <= radius + 0.5)   //this make so the explosion radius is a circle
					{
						if (WorldGen.TileEmpty(xPosition, yPosition))
						{
							...
							if (spedX == 0) spedX = 1;
							if (spedY == 0) spedY = 1;
							if (++cntr <= 100) Projectile.NewProjectile(position.X + x, position.Y + y, spedX, spedY, CalamityMod.ProjectileType("HyperiusBulletProj"), 90, 20, projectile.owner, 0.0f, 0);
						}
						else
						{
							spedX = Main.rand.Next(15) - 7;

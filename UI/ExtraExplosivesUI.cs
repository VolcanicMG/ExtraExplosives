						...
						if (Main.mouseLeftRelease && Main.mouseLeft && Craftable == true) //add a check here to see if its an item that can be combined and if it can produce it here
                        {
                            if (_vanillaItemSlot.Item.type == ModContent.ItemType<BulletBoomEmptyItem>() && _vanillaItemSlot2.Item.ammo == AmmoID.Bullet)
                            {
                                ...
                                if (_vanillaItemSlot.Item.stack >= 1 && _vanillaItemSlot2.Item.stack >= 10 && Main.netMode != NetmodeID.Server)
                                {
	                                ...
	                                spriteBatch.End();
	                                bool flag = false;
	                                foreach (Item checkItem in player.inventory)
	                                {
		                                if (checkItem.type == ModContent.ItemType<BulletBoomItem>())
		                                {
			                                ...
			                                if (modCheckItem.item.shoot == _vanillaItemSlot2.Item.shoot && modCheckItem.overStack <= 998)
			                                {
												...
												ItemText.NewText(checkItem, modCheckItem.overStack, true, false);
												flag = true;
												break;
			                                }
		                                }
	                                ...
	                                if (!flag)
	                                {
										if (Main.netMode == NetmodeID.MultiplayerClient)
										{
											...
											sb.Replace(" bullet", "");
											sb.Replace(" Bullet", "");
											int itemInt = Item.NewItem(player.getRect(), ModContent.ItemType<BulletBoomItem>(), 1);
											...
											Main.item[itemInt].SetNameOverride(sb.ToString() + " Bullet Boom");
											...
											//NetMessage.SendData(MessageID.InstancedItem, -1, -1, null);
											player.QuickSpawnClonedItem(Main.item[itemInt], 1);
											ItemText.NewText(_vanillaItemSlot2.Item, 1, true, false);
											//NetMessage.SendData(MessageID.SyncItem, -1, -1, null, itemInt);
										}
										else if(Main.netMode == NetmodeID.SinglePlayer)
										{
											...
											sb.Replace(" bullet", "");
											sb.Replace(" Bullet", "");
											int itemInt = Item.NewItem(player.position, ModContent.ItemType<BulletBoomItem>(), 1);
